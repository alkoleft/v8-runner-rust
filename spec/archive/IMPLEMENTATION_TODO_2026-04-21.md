# Архив TODO реализации Rust CLI на 2026-04-21

Этот файл является архивной копией `spec/IMPLEMENTATION_TODO.md` до очистки активного backlog.
Актуальный рабочий список находится в [spec/IMPLEMENTATION_TODO.md](../IMPLEMENTATION_TODO.md).
...
## Backlog по ADR (2026-04-21)

Детальная проработка ADR: `spec/ADR_DERIVED_BACKLOG.md`.

- [ ] `ADR-TASK-001`: Актуализировать активный backlog в `spec/IMPLEMENTATION_TODO.md`: сверить старые открытые EDT/MCP/runner-пункты с текущим кодом, закрытые отметить `[x]`, реально открытые оставить как рабочий backlog со ссылками на ADR.
...
- [ ] `ADR-TASK-010`: Добавить архитектурные guardrails для ADR-инвариантов (`ADR-0005`, `ADR-0006`, `ADR-0008`, `ADR-0009`, `ADR-0011`, `ADR-0017`): границы зависимостей use case, platform DSL boundary, workspace lock boundary, validation/docs для config contract, checklist изменения MCP surface.
