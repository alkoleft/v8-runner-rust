...
14. `v8project.local.yaml` является optional local overlay и не должен менять `source-set`, `format` или `builder`.
15. Tool extensions, включая `tools.client_mcp.extension`, не являются project `source-set`; их подготовка выполняется через общий механизм подготовки расширений на стадии `build`, а не на стадии `launch`.
