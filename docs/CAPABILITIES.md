...
Последняя факт-проверка: `2026-03-22` по свежей локальной сборке `cargo build`, актуальному CLI `--help`, `src/config/model.rs` и реальной MCP-поверхности запросов в `src/mcp/request.rs` / `src/mcp/service.rs`.
